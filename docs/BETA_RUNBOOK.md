...
5. Configure `AGENCY_BETA_MODE=1` and a nonempty comma-separated `TELEGRAM_ALLOWED_USER_IDS` of stable numeric user IDs. An empty/malformed invite list must refuse startup. Keep the shared Telegram account's username unchanged. Configure `REMEX_PROFILE_DB_PATH` and poll-state path outside tracked source (exact new variable to be confirmed by delivery slice). Set per-user budget/rate limits when integration lands.
6. Enumerate independent Python poller *trees*, including `runpy`/launcher children, rather than counting PIDs. Record each real working directory, code revision or runpy source, Python interpreter and active HTTPS connection without exposing args containing credentials. The deployment checkout previously observed at `42e6ad9` had a venv; the current running child observed under `theagency` had port-443 connectivity, but its executable source revision was **not proven** by that observation.
