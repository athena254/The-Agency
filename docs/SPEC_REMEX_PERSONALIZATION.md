...
- On normal private chat, pass the resolved, validated presentation name through *trusted Telegram-only context* to the orchestrator. Treat it as quoted display data, never as an instruction. Ordinary tool-capable and classic LLM paths should describe the same assistant name without changing the stable internal Butler identity. Group conversations use a separate memory scope for each chat/user pair, never the user's private-chat history; the default group name is Remex. Public anonymous HTTP requests must remain Remex/default and cannot set user preferences through payload context. Do not transform/forge the agent's actual output or disguise failures as successful replies.
...
Telegram handler accepts an optional injected ProfileStore for tests. Both polling and webhook production entry points construct a file-backed store from the same configuration; the polling bot registers `/name` in `TelegramBot.COMMANDS`, and each handler closes its store at shutdown. Existing stable ID check and chat allowlist run **before** profile writes. Do not introduce any public HTTP profile endpoint until authentication exists.
