...
- Trusted local caller passes an absolute repository path and a nonempty list of relative changed paths. Do not accept an HTTP request as authority. Reject absolute paths, `..`, paths escaping root via symlink, directories, oversized files, more than 64 files, path strings over 512 characters, and nonexistent files. Oversized batches produce one bounded failure record. Do not follow symlinks. No network, shell commands, subprocesses, generated code execution, installation, Git writes, or release. Inspect a quiescent trusted checkout: path validation followed by file read is not atomic, so v1 cannot promise protection against concurrent symlink swaps.
