...
async def test_list_findings_filters(test_evidence_store: EvidenceStore):
    ...
    assert len(await test_evidence_store.list_findings(FindingsFilter(target="staging"))) == 1
    assert len(await test_evidence_store.list_findings(FindingsFilter(severity=Severity.HIGH))) == 1
    assert (
        len(
            await test_evidence_store.list_findings(
                FindingsFilter(verification_status=VerificationState.VERIFIED)
            )
        )
        == 1
    )
    assert len(await test_evidence_store.list_findings(FindingsFilter(min_confidence=0.8))) == 1
    assert len(await test_evidence_store.list_findings(FindingsFilter(max_confidence=0.3))) == 1
...
async def test_evidence_append_only(test_evidence_store: EvidenceStore):
    finding = await test_evidence_store.add_finding(_finding())
    e1 = await test_evidence_store.add_evidence(
        EvidenceEntry(
            finding_id=finding.id, level=EvidenceLevel.LEVEL_1_STATIC, data={"tool": "nmap"}
        )
    )
    e2 = await test_evidence_store.add_evidence(
        EvidenceEntry(finding_id=finding.id, level=EvidenceLevel.LEVEL_2_BEHAVIORAL)
    )
...
async def test_evidence_requires_finding(test_evidence_store: EvidenceStore):
    with pytest.raises(EvidenceNotFoundError):
        await test_evidence_store.add_evidence(
            EvidenceEntry(finding_id="missing", level=EvidenceLevel.LEVEL_1_STATIC)
        )
...
async def test_latest_evidence_level(test_evidence_store: EvidenceStore):
    ...
    await test_evidence_store.add_evidence(
        EvidenceEntry(finding_id=finding.id, level=EvidenceLevel.LEVEL_1_STATIC)
    )
    await test_evidence_store.add_evidence(
        EvidenceEntry(finding_id=finding.id, level=EvidenceLevel.LEVEL_3_REPEATED)
    )
...
def test_level_manager_chain_validation():
    ...
    assert not m.validate_chain([EvidenceLevel.LEVEL_1_STATIC, EvidenceLevel.LEVEL_0_HYPOTHESIS])
    assert m.validate_chain([])
    assert m.strongest_level([]) is EvidenceLevel.LEVEL_0_HYPOTHESIS
    assert (
        m.strongest_level([EvidenceLevel.LEVEL_1_STATIC, EvidenceLevel.LEVEL_3_REPEATED])
        is EvidenceLevel.LEVEL_3_REPEATED
    )
